...
blackLower = np.array([90, 120, 77])
blackUpper = np.array([110, 230, 230])
...
def getData():
    ...
    if (read[0] == 0) and (read[1]) == 0 and (read[6] == 0) and (read[7] == 0):
        upAndDownMotor = int.from_bytes([read[2], read[3]], byteorder='big', signed=True)
        leftToRightMotor = int.from_bytes([read[4], read[5]], byteorder='big', signed=True)
        return upAndDownMotor, leftToRightMotor
    return 0
# ---------------------------
...
while True:
    ...
    if len(ptsXZJ) > 2 and len(ptsYZL) > 2:
        if timeTilPrediction > .1:
            ...
            print(upAndDown, " ======= left and right ->", LeftAndRight)
            ...
            sendData(int((predictedZ / .2) // 1), int((predictedX / 1.2) // 1))
